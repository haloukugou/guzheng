...
class sku {
    ...
    removeDisable($items, otherRowPrimeMultiply) {
        ...
        $items.each(function (index, item) {
            ...
            let can = settings.goodsList.some(element => {
                return element.descartes % (prime * otherRowPrimeMultiply) === 0;
            });
    ...
    multiplyIndex() {
        // 计算乘积对应的key
        let idx = -1;
        const hasProduct = this.settings.goodsList.some((item, index) => {
            idx = index;
            return item.descartes == this.multiplyResult;
        });
