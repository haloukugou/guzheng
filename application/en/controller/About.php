...
class About extends Base{
    ...
    public function view(){
		...
		$adRs = Db::name('ad')->where($pageWhere)->find();
		if(!empty($adRs)) $pageBanner = $adRs['thumb'];
		$aboutRs = Db::name($controller)->where('id',$id)->find();
    ...
    public function guestbook($id=0){
        if(request()->isPost()){
            ...
            if(!$captcha->check($verify)) $this->error(lang('c_verify_error'));

            //属性必填项判断
            $attWhere[] = ['tabledir','=','guestbook']; $attWhere[] = ['parentid','=',0]; $attWhere[] = ['lang','=',request()->module()];
            ...
            foreach ($attributeData as $key => $attRs) {
                ...
                $errorTips = lang('m_please_select').str_replace('*','',$attRs['title']);
                if(stripos($attRs['title'],'*') > 0 and empty($postAttValue)) $this->error($errorTips);
            }

            //交给模型处理
            $result = commonModelGuestbook::edit($id);
			if($result < 1) return $result->getError();
			$this->success(lang('c_guestbook_success'));
        }else{
        	$this->error(lang('c_submit_error'));
        }
    }
}
