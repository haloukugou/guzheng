...
class About extends Base{
    ...
    public function edit($id=0){
        ...
        if(!request()->isPost()){
            ...
            $this->assign('aboutParent',getChildSort(Db::name($tabledir)->where($aboutWhere)->select()));
			...
			$result = AboutModel::edit($tabledir,$id);
    ...
    public function index(){
		...
		$rs = ModuleModel::infoList($tabledir,$lang,10000,$field,$keyword,$order,$signid);
    ...
    public function update_all(){
        ...
        foreach($id as $i => $rs){
            Db::name('about')->update(['id'=>$id[$i],'sequence'=>$sequence[$i]]);
        }
        $this->success(lang('c_success'));
    }
}