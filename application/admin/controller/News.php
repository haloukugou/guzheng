...
class News extends Base{
    ...
    public function edit($id=0){
        ...
        if(!request()->isPost()){
			...
			//赋值新闻来源
			$siteinfoRs = Db::name('siteinfo')->where('lang',$lang)->find();
			$this->assign('siteinfoRs',$siteinfoRs);
			...
			$result = NewsModel::edit($tabledir,$id);
    ...
    public function update_all(){
        ...
        foreach($id as $i => $rs){
            Db::name('news')->update(['id'=>$id[$i],'title'=>$title[$i],'sequence'=>$sequence[$i]]);
        }
        $this->success(lang('c_success'));
    }
}