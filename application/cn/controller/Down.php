...
class Down extends Base{
    ...
    public function view(){
		...
		//更多附件
		$morefileurl = empty($rs['morefileurl']) ? '' : explode('|',$rs['morefileurl']);
        $this->assign('morefileurl',$morefileurl);
    ...
    public function download(){
        $id = input('id'); $controller = strtolower(request()->controller());
        $rs = Db::name($controller)->where('id',$id)->find();
        if(empty($rs)) $this->error(lang('c_param_error'));//参数错误
		//更新下载次数
		Db::name($controller)->where('id',$id)->setInc('downtimes',1);
