<?php /*a:4:{s:45:"../template/default/cn/mobile/about\view.html";i:1554291639;s:45:"../template/default/cn/mobile/public\css.html";i:1551600893;s:46:"../template/default/cn/mobile/public\head.html";i:1553997495;s:46:"../template/default/cn/mobile/public\foot.html";i:1551446328;}*/ ?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <?php if(is_array($list_nav) || $list_nav instanceof \think\Collection || $list_nav instanceof \think\Paginator): $i = 0; $__LIST__ = $list_nav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$listnavRs): $mod = ($i % 2 );++$i;?>
                    <i class="fa fa-angle-right mr-2"></i><a href="<?php echo url($lang_path.$listnavRs['urlroute'].'-'.$listnavRs['routesign'].$listnavRs['id'].'-1'); ?>"><?php echo htmlentities($listnavRs['title']); ?></a>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </div>
        </div>
    </div>
...
<div class="container mt-5">
    <div class="row">
        ...
        <div class="col-md-9 mb-4">
            <div class="row">
                <div class="col-md-12 detail">
                    <h1 class="h6 d-none d-md-block"><strong><?php echo htmlentities($rs['title']); ?></strong></h1><hr>
                    <?php echo $content1; ?>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-3 d-none d-md-block">
            <aside>
                <!--分类 开始-->
                <h6 class="font-weight-bold"><i class="fa fa-dot-circle font-weight-light small"></i> <?php echo htmlentities(app('lang')->get('v_about')); ?></h6>
                <ul class="nav nav-list flex-column mb-4">
                    <?php if(is_array($sortlist) || $sortlist instanceof \think\Collection || $sortlist instanceof \think\Paginator): $i = 0; $__LIST__ = $sortlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$oneRs): $mod = ($i % 2 );++$i;?>
                        <li>
                            <a href="<?php echo url($lang_path.$oneRs['urlroute'].'-'.$oneRs['routesign'].$oneRs['id'].'-1'); ?>"><?php echo htmlentities($oneRs['title']); ?></a>
                            <ul class="collapse show">
                                <?php $sortid = $oneRs['id']; $parentid = 0;$result = Db::name("about")->where("lang='$lang' and sortid=$sortid and signid=1")->limit("10")->order("sequence desc")->select();$__LIST__ = $result; if(is_array($__LIST__) || $__LIST__ instanceof \think\Collection || $__LIST__ instanceof \think\Paginator): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$aboutRs): $mod = ($i % 2 );++$i;?>
                                    <li><a class="<?php if($rs['id'] == $aboutRs['id']): ?>active<?php endif; ?>" href="<?php echo url($lang_path.$aboutRs['urlroute'].'-'.$aboutRs['routesign'].$aboutRs['id']); ?>"><?php echo htmlentities($aboutRs['title']); ?></a></li>
                                <?php endforeach; endif; else: echo "" ;endif; ?>
                            </ul>
                        </li>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                ...
                <!--分类 结束-->
                <!--最新信息 开始-->
                <h6 class="font-weight-bold"><i class="fa fa-dot-circle font-weight-light small"></i> <?php echo htmlentities(app('lang')->get('v_product_new')); ?></h6>
                <ul class="aside-list pl-0">
                    <?php $parentid = 0;$result = Db::name("product")->where("lang='$lang' and signid=1")->limit("5")->order("addtime desc")->select();$__LIST__ = $result; if(is_array($__LIST__) || $__LIST__ instanceof \think\Collection || $__LIST__ instanceof \think\Paginator): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$newproRs): $mod = ($i % 2 );++$i;?>
                    <li class="max-height-50">
                        <a href="<?php echo url($lang_path.$newproRs['urlroute'].'-'.$newproRs['routesign'].$newproRs['id']); ?>">
                            <img alt="<?php echo htmlentities($newproRs['title']); ?>" width="60" class="float-left img-fluid rounded mr-2 mt-1" src="<?php echo htmlentities($newproRs['thumb']); ?>" onerror="this.src='/skin/admin/img/nopic.png'">
                            <?php echo htmlentities($newproRs['title']); ?><br><span class="text-muted"><?php echo msubstr($newproRs['description'],0,52); ?></span>
                        </a>
                    </li>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                </ul>
