<?php /*a:4:{s:46:"../template/default/cn/mobile/jishu\index.html";i:1555339502;s:45:"../template/default/cn/mobile/public\css.html";i:1551600893;s:46:"../template/default/cn/mobile/public\head.html";i:1553997495;s:46:"../template/default/cn/mobile/public\foot.html";i:1551446328;}*/ ?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
    ...
    <div class="row">
        ...
        <div class="col-md-9 mb-4">
            <?php if(empty($list) || (($list instanceof \think\Collection || $list instanceof \think\Paginator ) && $list->isEmpty())): ?><div class="col-md-12 text-center"><?php echo htmlentities(app('lang')->get('v_nodata')); ?></div><?php endif; if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$rs): $mod = ($i % 2 );++$i;$fileurl = $root.$rs['fileurl']; ?>
                <div class="row">
                    <div class="col-md-3 col-sm-5 max-height-100"><a href="<?php echo url($lang_path.$rs['urlroute'].'-'.$rs['routesign'].$rs['id']); ?>"><img class="img-fluid rounded" src="<?php echo htmlentities($rs['thumb']); ?>" alt="<?php echo htmlentities($rs['title']); ?>" onerror="this.src='/skin/admin/img/nopic.png'"></a></div>
        ...
        <div class="col-md-3 d-none d-md-block">
            <aside>
                ...
                <!--分类 结束-->
                <!--热门信息 开始-->
                <h6 class="font-weight-bold mb-3"><i class="fa fa-dot-circle font-weight-light small"></i> <?php echo htmlentities(app('lang')->get('v_project_hot')); ?></h6>
                <ul class="aside-list pl-0">
                    <?php $parentid = 0;$result = Db::name("project")->where("lang='$lang' and signid=1")->limit("5")->order("hits desc")->select();$__LIST__ = $result; if(is_array($__LIST__) || $__LIST__ instanceof \think\Collection || $__LIST__ instanceof \think\Paginator): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hotproRs): $mod = ($i % 2 );++$i;?>
